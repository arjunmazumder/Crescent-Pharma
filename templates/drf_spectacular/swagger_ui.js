...
const responseInterceptor = (response, ...args) => {
  // Automatically authorize on successful login response
  if (response.url && response.url.includes('/api/auth/login/') && (response.status === 200 || response.status === 201)) {
    try {
      ...
      if (data && data.access) {
        ...
        setTimeout(() => {
          if (typeof ui !== 'undefined' && ui.authActions) {
            ...
            console.log("Swagger UI: Successfully and automatically authorized with JWT Access Token!");
          }
        }, 100);
      }
    } catch (err) {
      console.warn("Auto-authorize parsing error:", err);
    }
  }
